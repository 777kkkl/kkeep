<template>
	<div class="home-content">
		...
		<div class="statis-box">
			<div id="statis1" class="statis1 animate__animated" v-if="isAuth('shenqingjilu','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-zhangjie8"></span>
				</div>
				<div class="right">
					<div class="num">{{shenqingjiluCount}}</div>
					<div class="name">申请记录总数</div>
				</div>
			</div>
			<div id="statis2" class="statis2 animate__animated" v-if="isAuth('churujilu','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-chujia3"></span>
				</div>
				<div class="right">
					<div class="num">{{churujiluCount}}</div>
					<div class="name">出入记录总数</div>
				</div>
			</div>
			<div id="statis3" class="statis3 animate__animated" v-if="isAuth('shipinjiankong','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-qita3"></span>
				</div>
				<div class="right">
					<div class="num">{{shipinjiankongCount}}</div>
					<div class="name">视频监控总数</div>
				</div>
			</div>
			<div id="statis4" class="statis4 animate__animated" v-if="isAuth('baojingxinxi','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-liulan12"></span>
				</div>
				<div class="right">
					<div class="num">{{baojingxinxiCount}}</div>
					<div class="name">报警信息总数</div>
				</div>
			</div>
		</div>
		<!-- statis -->
		...
		<!-- 2 -->
		<div class="type2">
			<div id="churujiluChart1" class="echarts1 animate__animated" v-if="isAuth('churujilu','首页统计')"></div>
			<div id="churujiluChart2" class="echarts2 animate__animated" v-if="isAuth('churujilu','首页统计')"></div>
		</div>
	</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getshenqingjiluCount();
		this.getchurujiluCount();
		if(this.isAuth('churujilu','首页统计')){
			this.churujiluChat1();
		}
		if(this.isAuth('churujilu','首页统计')){
			this.churujiluChat2();
		}
		this.getshipinjiankongCount();
		this.getbaojingxinxiCount();
	...
	methods:{
		handleScroll() {
			let arr = [
				...
				{id:'churujiluChart1',css:'animate__fadeInUp'},
				{id:'churujiluChart2',css:'animate__fadeInUp'},
			]
		...
		getshenqingjiluCount() {
			this.$http({
				url: `shenqingjilu/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.shenqingjiluCount = data.data
				}
			})
		},
		getchurujiluCount() {
...
// 1234 竖
		churujiluChat1() {
			this.$nextTick(()=>{
				...
				var churujiluChart1 = echarts.init(document.getElementById("churujiluChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						churujiluChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							churujiluChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		churujiluChat2() {
			this.$nextTick(()=>{
				...
				var churujiluChart2 = echarts.init(document.getElementById("churujiluChart2"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						churujiluChart2.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							churujiluChart2.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		getshipinjiankongCount() {
			this.$http({
				url: `shipinjiankong/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.shipinjiankongCount = data.data
				}
			})
		},
		getbaojingxinxiCount() {
			this.$http({
				url: `baojingxinxi/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.baojingxinxiCount = data.data
				}
			})
		},
	}
