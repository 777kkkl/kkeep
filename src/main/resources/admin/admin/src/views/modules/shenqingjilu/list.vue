<template>
	<div class="main-content" :style='{"padding":"20px 30px","fontSize":"15px"}'>
			...
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex"}' >
					...
					<div :style='{"margin":"0 10px 0 0","display":"flex"}' class="select">
						<label :style='{"margin":"0 10px 0 0","color":"#999","display":"inline-block","width":"90px","lineHeight":"40px","fontSize":"15px","fontWeight":"500","height":"40px"}' class="item-label">是否通过</label>
				...
				<el-row class="actions" :style='{"padding":"0","margin":"10px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('shenqingjilu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia19" :style='{"margin":"0 2px","fontSize":"16px","color":"#fff","height":"34px"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('shenqingjilu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu12" :style='{"margin":"0 2px","fontSize":"16px","color":"#A40000","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('shenqingjilu','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						...
						label="信誉情况">
						<template slot-scope="scope">
							{{scope.row.xinyuqingkuang}}
						</template>
					</el-table-column>
					<el-table-column  :resizable='true' prop="peisongjietu" width="200" label="配送截图">
						<template slot-scope="scope">
							<div v-if="scope.row.peisongjietu">
								<img v-if="scope.row.peisongjietu.substring(0,4)=='http'" :src="scope.row.peisongjietu.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.peisongjietu.split(',')[0])">
								<img v-else :src="$base.url+scope.row.peisongjietu.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.peisongjietu.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('shenqingjilu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('shenqingjilu','出入记录')" @click="churujiluCrossAddOrUpdateHandler(scope.row,'cross','是','','[1]','已经记录了')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								出入记录
							</el-button>
							<el-button class="edit" v-if=" isAuth('shenqingjilu','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('shenqingjilu','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<churujilu-cross-add-or-update v-if="churujiluCrossAddOrUpdateFlag" :parent="this" ref="churujiluCrossaddOrUpdate"></churujilu-cross-add-or-update>
...
<script>
	...
	import churujiluCrossAddOrUpdate from "../churujilu/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				churujiluCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			churujiluCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.churujiluCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.churujiluCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.churujiluCrossaddOrUpdate.init(row.id,type);
				});
			},
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "shenqingjilu/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
