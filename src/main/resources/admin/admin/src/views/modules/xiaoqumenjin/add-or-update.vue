	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="upload" v-if="type!='info' && !ro.xiaoqutupian" label="小区图片" prop="xiaoqutupian" >
					<file-upload
						tip="点击上传小区图片"
						action="file/upload"
						:limit="3"
						:multiple="true"
						:fileUrls="ruleForm.xiaoqutupian?ruleForm.xiaoqutupian:''"
						@change="xiaoqutupianUploadChange"
					></file-upload>
				</el-form-item>
				<el-form-item class="upload" v-else-if="ruleForm.xiaoqutupian" label="小区图片" prop="xiaoqutupian" >
					<img v-if="ruleForm.xiaoqutupian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.xiaoqutupian.split(',')[0]" width="100" height="100">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.xiaoqutupian.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
...
<script>
	...
	export default {
		...
		methods: {
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'){
					...
					for (var o in obj){
						...
						if(o=='xiaoqutupian'){
							this.ruleForm.xiaoqutupian = obj[o];
							this.ro.xiaoqutupian = true;
							continue;
						}
						if(o=='kaifangshijian'){
				...
				this.$http({
					...
					if (data && data.code === 0) {
						...
						if(((json.xiaoqudizhi!=''&&json.xiaoqudizhi) || json.xiaoqudizhi==0) && this.$storage.get("role")!="管理员"){
							this.ruleForm.xiaoqudizhi = json.xiaoqudizhi
							this.ro.xiaoqudizhi = true;
						}
			...
			// 提交
			async onSubmit() {
					if(this.ruleForm.xiaoqutupian!=null) {
						this.ruleForm.xiaoqutupian = this.ruleForm.xiaoqutupian.replace(new RegExp(this.$base.url,"g"),"");
					}
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `xiaoqumenjin/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.xiaoqumenjinCrossAddOrUpdateFlag = false;
											this.parent.search();
			...
			xiaoqutupianUploadChange(fileUrls) {
				this.ruleForm.xiaoqutupian = fileUrls;
			},
		}
	};
</script>
